...
- Windows 服务端包和桌面包必须在原生 Windows 主机上构建，才能保持 `cgo` 和内置 MuPDF 可用。
- 如果需要一次性产出跨平台发布包，优先使用 GitHub Actions 发版流程，不要在 macOS/Linux 上本地交叉打 Windows 包。
...
该命令需要在原生 Windows 环境执行。若要生成正式发布包，优先使用 GitHub Actions 的 Windows runner。
