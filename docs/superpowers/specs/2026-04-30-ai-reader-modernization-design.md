...
**状态**: 主体已完成，部分设计已被 AI 助手调度中心方案替代（2026-05-01 审核）

## 当前完成状态

- **已完成**: 服务端会话持久化、会话列表与搜索、重命名/删除、消息持久化、多文献 pin、`@` 选择、自动标题、会话导出、上下文预算、摘要器、证据引用渲染、入口注入和 settings 配置项。
- **已演进**: “AI 伴读”已经升级为更通用的 AI 助手页面；“严格证据模式”在后续设计中演进为“内部搜索/外部搜索”和 orchestrator 工具调度。
- **实现差异**: 流式输出实际采用 NDJSON 事件；原文中提到的 SSE 命名保留为历史描述，不再作为当前接口定义。
- **保留边界**: 分支编辑、star/归档、文件夹、多账号共享等仍属于 v2 或暂不做范围。
