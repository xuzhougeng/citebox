...
普通模式下，钉住文献的摘要和正文按 `context_budget_tokens` 的剩余预算分配；系统提示、问题、已有对话摘要和用户附件先计入预算，工具结果也在装配正文之前预留预算，再在各篇文献间分配可用空间。存在钉住文献时，工具结果最多使用剩余文本预算的一半，其余留给钉住正文和历史。单篇摘要最多 4000 字符；正文没有独立的字符数上限，按分配到的剩余文本预算尽量完整带入。正文超出预算时按常见中英文章节标题抽样，优先覆盖 Methods / STAR Methods、统计分析、数据与代码可用性等章节，并保留文末片段；无法识别标题时使用分布式窗口。抽样最多 8 段，标注字符位置、实际带入量和非完整全文边界。历史消息继续使用剩余空间。超长工具结果也会按预算抽样并披露；用户问题或附件本身超出预算时，不会截断这些显式输入。文本预算是启发式估算，不包含供应商的图片 token 计费。
...
- `context`：可选上下文对象，支持 `source`、`paper_id`、`paper_ids`、`figure_id`、`figure_ids`、`excerpts`、`auto_attach_figures`。用于指定当前文献、对比文献或图片上下文。当 `intent_hint == "library_search"` 且 `paper_ids` 非空时（典型场景：用户同时输入 `@Library @<paper>`），后端会把候选集裁剪到该 PaperIDs 集合内。
  ...
  - `context.auto_attach_figures`：可选布尔值，默认 `false`。开启后，在没有显式 `figure_id` / `figure_ids` 的情况下，按已钉文献顺序轮流选取顶层图片（每篇先取一张，再轮到下一张），合计最多 4 张；无图或读取失败的文献跳过，不包含子图。显式选择优先且会去重，仍受现有最多 8 张及图片字节预算限制。仅使用库中已提取图片，不渲染整个 PDF。图像生成意图不使用此开关。AI 页提供本轮附图开关，刷新页面后恢复关闭。
...
- 非流式 AI 阅读
- 返回完整 JSON 结果
- 图片查看器的批量解读串行复用此接口，每次只处理一张顶层图片；成功后通过 `PUT /api/figures/{id}` 写入该图笔记，默认跳过已有笔记，也可选择全部并追加或覆盖。单张失败会继续处理其他图片，结束后汇总；中断会取消当前等待的解读请求，保留已保存的笔记，不再启动下一张。已开始的笔记保存请求允许完成。
...
- 前端切换语言时等待保存成功后才更新本地缓存并刷新；保存失败保留原语言并提示重试。未登录的登录页收到 401 时仍可切换本地语言。
...
Conversation turns now plan retrieval using the current question, up to six recent messages, the conversation summary, and pinned paper titles. Explicit intent hints and paper scope take precedence. The planner resolves follow-up references and produces bounded literal search terms shared by library search and paper reading. External search retains its source-specific query planner. Planning failures fall back to the existing rule route; cancellation stops the turn.
...
### Local evidence provenance and judgments

Local citation snippets add `origin`: `original`, `user_note`, `ai_note`, or `mixed_note`. Original title/abstract/body matches rank ahead of derived notes. Existing AI note source links are recognized; fields combining management notes and AI content are conservatively marked as mixed. Unmarked historical notes remain user notes, never original-paper evidence. Strict local evidence retrieval excludes notes entirely.

Conversation tool citations add `verdict` (`supports`, `partially_supports`, `contradicts`, `related_only`, `insufficient`, `unassessed`, `note_only`) and optional `assessment`. A bounded subagent call judges up to 20 local original snippets with a 20-second timeout. Invalid indices/verdicts cannot create citations or promote notes; failures preserve snippets as unassessed. These are provisional judgments, not proof that the full paper supports a claim. Card snippets carry `origin` and `verdict` for localized display. The existing tool-call store records `evidence_judge`; no schema migration is needed.

### Citation source snapshots and note export

New local citations carry optional `page`, `source_url`, and `source_revision` (`sha256:` hash of the retrieved source field). These are resolved at retrieval time and persisted in the existing citations JSON. A page is assigned only when the complete normalized excerpt occurs on exactly one stored PDF page; missing, cross-page, or ambiguous matches have no page. Local body links open the existing PDF viewer at the known page. External citations retain their external source URL and never inherit local-paper revisions.

Appending an AI answer to paper notes now includes a readable snapshot appendix for referenced citations: original title, excerpt, known/unknown page, source link, revision, origin and assessment. Saving does not re-read current paper text to reconstruct historical evidence. Snapshots survive conversation deletion and remain in note exports; conversation Markdown export also includes them. Stopped/failed answers cannot be saved as completed notes. Existing answers without citations keep the prior behavior. Malformed stored citation JSON is reported rather than silently dropping evidence.

### Shared reading context selection
