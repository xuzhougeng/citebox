...
3. command 只能从以下列表中选择：
   "/help", "/status", "/reset", "/recent", "/figures", "/search", "/search-papers", "/search-figures", "/paper", "/figure", "/random", "/ask", "/note", "/interpret"
...
8. 如果用户明确要求随机看一张图，例如“随机来一张图”“随便发张图”，就返回 "/random"。
9. 如果当前已有文献上下文，而用户是在继续追问当前文献内容，用 "/ask"。
10. 如果当前已有图片上下文，而用户是在要求解释当前图片，用 "/interpret"。
11. 如果用户是在追加笔记，用 "/note"。
12. 识别不准时优先选择最保守、最合理的命令；完全无法判断时返回 "/help"。
