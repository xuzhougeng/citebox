...
2. 识别 PDF 复制文本中的软换行：段落内由 PDF 排版产生的单个换行应当按同一句或同一自然段合并，不要把每一行翻译成独立段落。
3. 只有原文中的连续空行、列表项、标题或明显段落边界才保留为译文段落或列表边界；不要保留普通视觉换行。
4. 保留原文中的数字、单位、缩写、专有名词和列表层级。
5. 如果原文已经是目标语言，也请只做必要润色后输出正文。
